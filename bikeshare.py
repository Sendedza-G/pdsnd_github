...
def city_input():
    ...
    while True:     # for handling the unexpected input by user
            ...
            # error handled by implementing 'else' and provided another option to input data
                ...
                city = input('Please choose the city for which you would like to view it\'s Stats: ')
                city = city.lower()
    return city
...
def get_time():
    ...
    while True:
        ...
        if times == "day":
            ...
            return "none"
        period = input("\n Please provide us with 'month', 'day' of the week, or none (no) \n").lower()
...
def month_day_info(df, day_m):     # Asks the user for a month and a day of month,
    ...
    if day_m == "day_of_month":
        ...
        while (True):
            ask = """ \n Which day do want to view it\'s information? \n
            Please choose the number between 1 and 7 """
...
def disp_raw_data(df):
    ...
    see_data = input("\nwould you like to see rows of the data used in our stats? Please select by typing 'yes' or 'no' \n").lower()
    while True:
        if see_data == 'no':
            return
        if see_data == 'yes':
        ...
        see_data = input("\n Still want to see more information? Please select by typing 'yes' or 'no' \n").lower()
...
def main():
    ...
    # Restarting option
    restart = input("\n * Do you want to perform another analysis? Type \'yes\' or \'no\'.\n")
